...
import { getServiceDetailSchemas } from "@/components/services/service-detail-schema";

export default function ServiceDetailPage({ params }: { params: { slug: string } }) {
  const service = ALL_13_SERVICES_DETAILED.find((s) => s.slug === params.slug);
  if (!service) notFound();

  const { serviceSchema, breadcrumbSchema } = getServiceDetailSchemas(service, siteUrl);
